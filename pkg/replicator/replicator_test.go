...
func TestValidateReplication(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "invalid pattern in list - valid one first matches",
			sourceNamespace: "production",
		...
		{
			name:            "invalid pattern in list - invalid one first, causes error",
			sourceNamespace: "production",
			sourceAllowlist: "[invalid,staging",
			targetNamespace: "staging",
			want:            false,
			wantErr:         true,
		},
		{
			name:            "allowlist with empty entries between commas",
			sourceNamespace: "production",
			sourceAllowlist: "staging,,development",
			targetNamespace: "development",
			want:            true,
			wantErr:         false,
		},
